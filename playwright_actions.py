# --- ファイル: playwright_actions.py (アプローチ2対応・URL返却・完全コード) ---
import asyncio
import logging
import os
import time
...
async def _extract_emails_from_page_async(context: BrowserContext, url: str, timeout: int) -> List[str]:
        ...
        if emails_found:
            ...
            try:
                ...
                with open(mail_file_path, "a", encoding="utf-8") as f:
                    for email in unique_emails_list: f.write(email + '\n')
    ...
    except Exception as e: logger.warning(f"メール抽出エラー ({url}): {type(e).__name__} - {e}", exc_info=False); logger.debug(f"Traceback:", exc_info=True); return []
    finally:
        if page and not page.is_closed():
            try: await page.close()
            except Exception: pass

# --- Locator試行ヘルパー関数 (変更なし) ---
async def try_locators_sequentially(
    ...
    start_time = time.monotonic(); successful_locator: Optional[Locator] = None; successful_hint: Optional[Dict[str, Any]] = None
    attempt_logs: List[Dict[str, Any]] = []; hint_count = len(hints) if hints else 1
    base_validation_timeout = max(500, min(2000, overall_timeout // max(1, hint_count)))
    logger.info(f"[{action_name}] Trying {len(hints)} locator hints (timeout: {overall_timeout}ms, validation_timeout: {base_validation_timeout}ms)...")
    hints.sort(key=lambda x: {"high": 0, "medium": 1, "low": 2}.get(x.get("confidence", "low"), 2))
    for i, hint in enumerate(hints):
        elapsed_ms = (time.monotonic() - start_time) * 1000
        if elapsed_ms >= overall_timeout: logger.warning(f"[{action_name}] Overall timeout"); break
        remaining_ms = overall_timeout - elapsed_ms; current_validation_timeout = min(base_validation_timeout, max(100, int(remaining_ms * 0.8)))
        hint_type = hint.get("type"); hint_value = hint.get("value"); hint_name = hint.get("name"); hint_level = hint.get("level")
        hint_common_selector = hint.get("common_selector"); hint_index = hint.get("index"); hint_confidence = hint.get("confidence", "low")
        ...
        attempt_log = {"hint": hint, "status": "Skipped", "reason": "No locator generated"}; locator_to_try: Optional[Locator] = None
        try:
            options = {}
            if hint_type == "role_and_text": # 修正済み: valueかroleキーを見る
                 actual_role = hint_value if isinstance(hint_value, str) else hint.get("role")
                 if not isinstance(actual_role, str) or not actual_role: raise ValueError("Role missing/invalid")
                 if isinstance(hint_name, str):
                     if hint_name.startswith('/') and hint_name.endswith('/') and len(hint_name) > 2:
                          try: pattern_part = hint_name[1:-1]; flags = 0; # ... (regex logic) ...
                          except re.error as re_err: options["name"] = hint_name; logging.warning(f"Invalid regex: {re_err}")
            ...
            elif hint_type == "nth_child" and isinstance(hint_common_selector, str) and isinstance(hint_index, int) and hint_index >= 0: locator_to_try = target_scope.locator(hint_common_selector).nth(hint_index)
            else: attempt_log["reason"] = f"Invalid/unknown hint: {hint}"; logging.warning(f"    -> {attempt_log['reason']}")
            ...
            if locator_to_try:
                ...
                if count == 1:
                    ...
                    except PlaywrightTimeoutError: logging.debug(f"    -> Not visible.")
                    except Exception as vis_err: logging.warning(f"    -> Visibility check error: {vis_err}")
                    attempt_log["is_visible"] = is_visible
                    if is_visible: successful_locator = locator_to_try; successful_hint = hint; attempt_log["status"] = "Success"; attempt_log["reason"] = "Found unique/visible."; logging.info(f"    -> Success!"); attempt_logs.append(attempt_log); break
                    else: attempt_log["status"] = "Fail"; attempt_log["reason"] = "Found but not visible."; logging.info(f"    -> Fail: Not visible.")
                elif count > 1: attempt_log["status"] = "Fail"; attempt_log["reason"] = f"Multiple elements ({count})."; logging.info(f"    -> Fail: Multiple.")
                else: attempt_log["status"] = "Fail"; attempt_log["reason"] = "Not found."; logging.info(f"    -> Fail: Not found.")
            else: attempt_log["status"] = "Fail"
        except ValueError as ve: attempt_log["status"] = "Error"; attempt_log["reason"] = f"ValueError: {ve}"; logging.warning(f"    -> ValueError: {ve}")
    ...
    if successful_locator: logger.info(f"[{action_name}] Located element via hint: {successful_hint}")
    else: logger.warning(f"[{action_name}] Failed to locate element via hints.")
    return successful_locator, successful_hint, attempt_logs


# --- execute_actions_async 本体 (引数と戻り値、URL更新を修正) ---
async def execute_actions_async(
    page: Page, # ★ initial_page から page に変更 ★
    actions: List[Dict[str, Any]],
    api_request_context: APIRequestContext,
    default_timeout: int
) -> Tuple[bool, List[Dict[str, Any]], str]: # ★ 戻り値に str (最終URL) を追加 ★
    ...
    for i, step_data in enumerate(actions):
        ...
        option_value = step_data.get("option_value"); action_wait_time = step_data.get("wait_time_ms", default_timeout)
        ...
        try:
            ...
            # ★ final_url を更新し、current_base_url もそれを使う ★
            ...
            if action == "switch_to_iframe":
                ...
                # ★ iframe切り替え後もURLを更新しておく (通常は変わらないはずだが念のため) ★
                final_url = root_page.url
                continue
            ...
            if action in ["wait_page_load", "sleep", "scroll_page_to_bottom"]:
                 if action == "wait_page_load": await root_page.wait_for_load_state("load", timeout=action_wait_time); logger.info("Page load complete.")
                     ...
                     final_url = root_page.url; continue # ★ sleep後もURL更新 ★
                 ...
                 final_url = root_page.url; continue # ★ スクロール後もURL更新 ★

            # --- 要素操作準備と要素特定ロジック (変更なし) ---
            target_element: Optional[Locator] = None; found_elements_list: List[Tuple[Locator, Union[Page, FrameLocator]]] = []
            found_scope: Optional[Union[Page, FrameLocator]] = None; successful_hint_info: Optional[Dict[str, Any]] = None
            ...
            if needs_single_element and found_scope:
                if id(found_scope) != id(current_target): logger.info(f"Scope updated to: {type(found_scope).__name__}"); iframe_stack.append(current_target); current_target = found_scope
            ...
            if action == "click":
                if not target_element: raise PlaywrightError("Click failed: Element not located.")
                logger.info("Clicking element...")
                context_for_click = root_page.context; new_page: Optional[Page] = None
                try:
                    async with context_for_click.expect_page(timeout=config.NEW_PAGE_EVENT_TIMEOUT) as new_page_info: await target_element.click(timeout=action_wait_time)
                ...
                results.append({**step_result_base, "status": "success", **action_result_details})
                 ...
                 if not target_element: raise PlaywrightError("Input failed: Element not located.")
                 if value is None: raise ValueError("Input requires 'value'.")
                 input_value_str = str(value); logger.info(f"要素に '{input_value_str[:50]}{'...' if len(input_value_str) > 50 else ''}' を入力...")
                 try:
                     ...
                     await asyncio.sleep(0.1); logger.info("入力成功")
                     action_result_details["value"] = value; results.append({**step_result_base, "status": "success", **action_result_details})
                 except Exception as e: logger.error(f"Input error: {e}", exc_info=True); raise PlaywrightError(f"Failed input: {e}") from e

            elif action == "hover":
                 if not target_element: raise PlaywrightError("Hover failed: Element not located.")
                 await target_element.hover(timeout=action_wait_time); logger.info("Hover success.")
                 results.append({**step_result_base, "status": "success", **action_result_details})

            elif action == "get_inner_text":
                 if not target_element: raise PlaywrightError("Get innerText failed: Element not located.")
                 ...
                 if not target_element: raise PlaywrightError("Get textContent failed: Element not located.")
                 ...
                 if not target_element: raise PlaywrightError("Get innerHTML failed: Element not located.")
                ...
                if attribute_name.lower() == 'href' and attr_value is not None:
                     ...
                     except Exception as url_e: logger.error(f"URL変換エラー: {url_e}"); processed_value = f"Error: {url_e}"
                     ...
                     except Exception as pdf_e: logger.error(f"PDF処理エラー: {pdf_e}"); pdf_text_content = f"Error: {pdf_e}"
                     ...
                     try:
                         ...
                         elif absolute_url.lower().endswith('.pdf'): scraped_content = "Warning: Link is PDF, use attribute_name='pdf'"
                         else: scraped_content = f"Warning: Invalid URL scheme or not HTTP/S: {absolute_url}"
                     except Exception as content_e: logger.error(f"コンテンツ取得エラー: {content_e}"); scraped_content = f"Error: {content_e}"
                logger.info(f"Got attribute '{attribute_name}': '{str(processed_value)[:100]}...'")
                ...
                if not found_elements_list: logger.warning(f"No elements for '{selector}'."); action_result_details["results_count"] = 0
                    ...
                    if attribute_name.lower() in ['href', 'pdf', 'content', 'mail']:
                        ...
                        async def process_single_element_for_href_related(locator: Locator, index: int, base_url: str, attr_mode: str, sem: asyncio.Semaphore) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[List[str]]]:
                            original_href: Optional[str] = None; absolute_url: Optional[str] = None; pdf_text: Optional[str] = None; scraped_text: Optional[str] = None; emails_from_page: Optional[List[str]] = None
                            async with sem:
                                try:
                                    ...
                                    if attr_mode == 'pdf' and absolute_url.lower().endswith('.pdf'):
                                         ...
                                         success_content, content_or_error = await get_page_inner_text(current_context, absolute_url, action_wait_time); scraped_text = content_or_error
                                ...
                                except (PlaywrightError, Exception) as e: error_msg = f"Error: {type(e).__name__}"; return absolute_url or f"Error processing: {original_href}", error_msg if attr_mode=='pdf' else None, error_msg if attr_mode=='content' else None, [error_msg] if attr_mode=='mail' else None
                        ...
                        for url_res, pdf_res, content_res, mails_res in results_tuples: # 変数名変更
                        ...
                        action_result_details["url_list"] = url_list # URLリストは常に追加
                        ...
                        action_result_details.update({"attribute_list": generic_list, "results_count": len(generic_list)})
                    action_result_details["attribute"] = attribute_name # 属性名を記録
                results.append({**step_result_base, "status": "success", **action_result_details})
                ...
                if not target_element: raise PlaywrightError("Wait visible failed: Element not located.")
                logger.info("Element visibility confirmed."); results.append({**step_result_base, "status": "success", **action_result_details})
                  ...
                  elif option_type == 'index': value_to_select = {'index': int(option_value)}
                  else: value_to_select = {'label': str(option_value)}
                  selected_values = await target_element.select_option(value_to_select, timeout=action_wait_time)
                  ...
                  screenshot_path = os.path.join(config.DEFAULT_SCREENSHOT_DIR, safe_filename); os.makedirs(config.DEFAULT_SCREENSHOT_DIR, exist_ok=True)
                  ...
                  action_result_details["filename"] = screenshot_path # 結果には実際のパスを記録
                  results.append({**step_result_base, "status": "success", **action_result_details})
                ...
                if action not in known_actions:
                     ...
                     results.append({**step_result_base, "status": "skipped", "message": f"Undefined action: {action}"}); continue
        ...
        # --- ステップごとのエラーハンドリング ---
            ...
            try:
                ...
                elif page and not page.is_closed(): current_url_on_error = page.url # root_pageがなければpageを試す
            except Exception: pass
